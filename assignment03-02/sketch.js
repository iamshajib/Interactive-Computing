...
function preload(){
    ...
    wallPic = loadImage("images/wallOne.png");
...
//    image(parking, 1150, 7.5+height/2);
    image(borderW, width/2, 0);
    image(borderW, width/2, 700);
    image(borderH, 0, height/2);
    image(borderH, 1100, height/2);
  ...
  move() {
    ...
    if (keyIsDown(65)) {
        ...
        for (var i = 0; i < theWalls.length; i++) {            
            if(this.down > theWalls[i].top && this.top < theWalls[i].down) {
                if (this.left-2.5 < theWalls[i].right && this.right > theWalls[i].right) {
                    wallLeft = true;
                }
            }
        }
    ...
    if (keyIsDown(68)) {
      var wallRight = false;
        ...
        for (var i = 0; i < theWalls.length; i++) {            
            if(this.down > theWalls[i].top && this.top < theWalls[i].down) {
                if (this.right+2.5 > theWalls[i].left && this.left < theWalls[i].left) {
                    wallRight = true;
                }
            }
        }
    ...
    if (keyIsDown(87)) {
        ...
        var wallUp = false;
          ...
          for (var i = 0; i < theWalls.length; i++) {            
            if(this.right > theWalls[i].left && this.left < theWalls[i].right) {
                if (this.top-2.5 < theWalls[i].down && this.down > theWalls[i].down) {
                    wallUp = true;
                }
            }
        }
    ...
    if (keyIsDown(83)) {
        ...
        for (var i = 0; i < theWalls.length; i++) {            
            if(this.right > theWalls[i].left && this.left < theWalls[i].right) {
                if (this.down+2.5 > theWalls[i].top && this.top < theWalls[i].top) {
                    wallDown = true;
                }
            }
        }
